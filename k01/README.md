...
65~79 linear searchでkeyと一致するIDを探す。
73 一致するIDが見つかったらそのインデックスを返す。
78 一致するIDが見つからなかったら-1を返す。

81~106 binary searchでkeyと一致するIDを探す。
89 配列の真ん中のインデックスを求める。
91~94 一致するIDが見つかったらそのインデックスを返す。
95~98 真ん中のインデックスがkeyより小さい場合,left=pos+1する。
99~102 真ん中のインデックスがkeyより大きい場合,right=pos-1する。
105 一致するIDが見つからなかったら-1を返す。

...
## ソースコードの説明
87 入替回数を初期化する。
90~95 隣と比較して左のほうが大きい場合、左右を入れ替える。
110~145 要素数が1より多い場合行う。
134~138 i>=jでなければiとjを入れ替える。
140~142 pivptとjの値を入れ替える。
143 左側をQuickSort.
144 右側をQuickSort.
...
```
$ ./k04
bash: ./k04: No such file or directory

実行しようとしてもファイルかディレクトリがないと表示されます。
cd k04で移動しているのですができません。
どうすれば実行できるか教えてもらえないでしょうか。
```
